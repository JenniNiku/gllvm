...
#' @description Plot latent variables and their corresponding coefficients (biplot).
#'
#' @param object   An object of class 'gllvm'
#' @param biplot   \code{TRUE} if both latent variables and their coefficients are plotted, \code{FALSE} if only LVs
#' @param ind.spp  Number of LV coefficients included in the biplot.
...
#'
#' @details
#' Construct a scatter plot of two latent variables, i.e. an ordination plot. If only one latent
...
#'ordplot.gllvm(fit, biplot = TRUE, ind.spp = 10)
#'}
#'@export


ordplot.gllvm <- function(object, biplot = FALSE, ind.spp = NULL, alpha = 0.5, main = NULL, which.lvs = c(1, 2), jitter = FALSE, ...) {
