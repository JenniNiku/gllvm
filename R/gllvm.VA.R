...
gllvm.VA <- function(y, X = NULL, T = NULL, family = "poisson", num.lv = 2, max.iter = 200, eps = 1e-4, row.eff = TRUE,
  ...
  if(!is.null(T)) {
    ...
    T.new <- NULL
    for (i in 1:num.T) {
      if(!is.factor(T[,i])) { T.new <- cbind(T.new,scale(T[,i])); colnames(T.new)[dim(T.new)[2]] <- names(T)[i] }
      if(is.factor(T[,i])){
        dum <- model.matrix(~T[,i]-1)
        colnames(dum) <- paste(names(T)[i],levels(T[,i]),sep="")
        T.new <- cbind(T.new,dum)
      }
    }
    T <- data.matrix(T.new); num.T <- dim(T)[2];
  }
  ...
  out.list <-  list(y = y, X = X, T = T, num.lv = num.lv, row.eff = row.eff, logLik = -Inf, family = family, offset=offset)
  ...
  while(n.i<=n.init){
    ...
    while((err > (1 + eps) || err < (1 - eps)) && iter <= max.iter) {
      ...
      if(Lambda.struc == "unstructured" & iter <= diag.iter) {
        ...
        if(iter == 1)  new.lambda <- lambda <- matrix(1,n,num.lv)
      ...
      ll0 <- function(x,v=NULL,lambda=NULL,phi,zeta=NULL) {
        ...
        if(!is.null(T) && get.fourth) mu.mat <- mu.mat + matrix((kronecker(T,X) %*% new.inter),n,p)
        if(!is.null(T) && get.trait) mu.mat <- mu.mat +  new.trait %*% t(T)    #!!!
        ...
        if(family == "ordinal") {
          out1 <- matrix(NA,n,p)
          for(j in 1:p) {
            out1[y[,j] == 1,j] <- pnorm(new.zeta[j,1]-mu.mat[y[,j] == 1,j],log=TRUE)
      ...
      grad.mod <- function(x,v=NULL,lambda=NULL,phi,zeta=NULL) {
        ...
        grad.theta <- grad.beta0 <- grad.env <- grad.inter <- grad.row.params <-grad.trait<- NULL
        ...
        if(family=="binomial") {
          ...
          if(!is.null(T) && get.fourth) {
            sum1 <- c(dnorm(eta.mat) * (y - trial.size * probs) / (probs * (1 - probs) + 1e-5)) * kronecker(T,X)
            grad.inter <- c(grad.inter,colSums(sum1))
          }
          if(!is.null(T) && get.trait) {
            grad.trait<-colSums(dnorm(eta.mat) * (y - trial.size * probs) / (probs * (1 - probs) + 1e-5))%*%T
          }
        ...
        return(c(grad.theta, grad.beta0, grad.env, grad.inter, grad.row.params,grad.trait))
      }
      ...
      if(constrOpt) {
        ...
        if(!is.null(X)){
          if(is.null(T)){
            ...
            } else{ B2=X; for(i in 1:(p-1)) B2=rbind(B2,X)}
        }
        ...
        if(!is.null(T)){
          ...
          if(get.trait){
            for(i in 1:p){
              Bt<-rbind(Bt,matrix(T[i,],n,num.T,byrow = TRUE))
          }}
        }
      ...
      if(!inherits(q, "try-error")) {
        ...
        if(iter > 1 && current.loglik > q$value) { if(trace) cat("Optimization did not improve estimates of model parameters on iteration step ",iter,"\n"); new.theta <- theta; new.beta0 <- beta0; new.env <- env; new.inter <- inter; new.row.params <- row.params}
        else {
          if(trace) cat("Model parameters updated", "\n")
          x2 <- q$par; #plot(q$par)
          if(num.lv > 0) { new.theta <- matrix(x2[1:(p * num.lv)],p,num.lv); x2 <- x2[-(1:(p * num.lv))]; new.theta[upper.tri(new.theta)] <- 0 }
          new.beta0 <- x2[1:p]; x2 <- x2[-(1:p)]; #plot(new.beta0)
          new.env <- NULL
          if(!is.null(X)) {
            if(is.null(T)) { new.env <- matrix(x2[1:(p * num.X)],p,num.X); x2 <- x2[-(1:(p * num.X))] }
            else {
              new.env <- x2[1:num.X]; x2 <- x2[-(1:num.X)]
            }
          }
          new.inter <- NULL; if(!is.null(T) && get.fourth) { new.inter <- x2[1:(num.X * num.T)]; x2 <- x2[-(1:(num.X * num.T))] }
          new.row.params <- NULL; if(row.eff) { new.row.params <- x2[1:n]; x2 <- x2[-(1:n)] }
      ...
      else { new.theta <- theta; new.beta0 <- beta0; new.env <- env; new.inter <- inter; new.row.params <- row.params; new.trait<-trait }
      ...
      if(family=="negative.binomial") {
        grad.phi <- function(x,v=NULL,lambda=NULL,phi,zeta = NULL) {
          ...
          grad.theta <- grad.beta0 <- grad.env <- grad.inter <- grad.row.params <- NULL
      ...
      if(family == "ordinal") {
        eta.mat <- matrix(new.beta0,n,p,byrow=TRUE) + offset
        if(!is.null(T) && get.fourth) { eta.mat <- eta.mat + matrix((kronecker(T,X) %*% new.inter),n,p); new.env1=matrix(rep(new.env,each=p),nrow=p) } else{ new.env1=new.env }
        if(!is.null(T) && get.trait) eta.mat <- eta.mat +  new.trait %*% t(T)
        if(!is.null(X)) eta.mat <- eta.mat + X %*% t(new.env1)
        ...
        func.zetaj <- function(cw.zeta, j) { ## Exclude the first column of zeta, which is fixed at 0
          ...
          out <- out + sum(pnorm(zeta0[1]-eta.mat[which(y[,j] == 1),j],log=TRUE))
      ...
      if(num.lv > 0) {
        ...
        if(constrOpt)  {
          ...
          if(!is.null(X) && !is.null(T)){ new.env1 <- matrix(rep(new.env,each=p),nrow=p)} else new.env1=new.env
          if(!is.null(T) && get.trait) new.trait1=matrix(rep(new.trait,each=n),nrow=n)
          etas <- matrix(new.beta0,n,p,byrow=TRUE)  + offset
          if(!is.null(X)) etas <- etas + X %*% t(new.env1)
          if(!is.null(T) && get.fourth) etas <- etas + matrix((kronecker(T,X) %*% new.inter),n,p)
        ...
        if(!is.null(T)){ if(get.fourth) eta.mat <- eta.mat + matrix((kronecker(T,X) %*% new.inter),n,p);
        new.env1=matrix(rep(new.env,each=p),nrow=p)
        if(get.trait) new.trait1=matrix(rep(new.trait,each=n),nrow=n)
        } else{ new.env1 <- new.env; new.trait1<-new.trait }
        if(!is.null(T) && get.trait) eta.mat <- eta.mat +  new.trait1 %*% t(T)
        if(!is.null(X)) eta.mat <- eta.mat + X %*% t(new.env1)
        if(row.eff) eta.mat <- eta.mat + matrix(new.row.params,n,p)
      ...
      beta0 <- new.beta0; env <- new.env; inter <- new.inter; theta <- new.theta;  phi <- new.phi; trait<-new.trait
    ...
    if(n.i==1 || out.list$logL < current.loglik){
      out.list$logLik<-current.loglik
      ...
      if(!is.null(X)) {
        if(is.null(T)) {
          ...
          env.coefs <- env
          names(env.coefs) <- colnames(X)
          out.list$coef$Xcoef=env.coefs
        }
      }

      if(!is.null(T) && get.fourth) {
        fourth<-inter
        inter.coefs <- matrix(inter,num.T,num.X,byrow=TRUE)
        rownames(inter.coefs) <- colnames(T);  colnames(inter.coefs) <- colnames(X); out.list$coef$fourth = inter.coefs
      }
      if(!is.null(T) && get.trait) {
        T.coefs <- trait
        names(T.coefs) <- colnames(T)
        out.list$coef$Tcoef=T.coefs
      }

      if(row.eff) { names(row.params) <- rownames(y); out.list$coef$row.params <- row.params }
