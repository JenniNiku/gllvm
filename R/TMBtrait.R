  ...
  while(n.i <= n.init){
    ...
    if(!is.null(randomX)){
      ...
      if(!is.null(colMat) && all(dim(colMat)!=1)){
        ...
        blocks = append(list(matrix(c(p,blocksp))), blocks)
      ...
      if((method %in% c("VA", "EVA")) && !is.null(randomX)){
        ...
        if(Ab.struct%in%c("blockdiagonal","diagonal")){
          ...
          if(ncol(colMat)!=p){
            ...
            for(j in 1:(p*ncol(xb)-1)){
              for(r in (j+1):(p*ncol(xb))){
                Abs[[1]][r,j] = Ab[1];
                Ab <- Ab[-1]
              }}
          }else{
