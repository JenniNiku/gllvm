...
predictSR.gllvm <- function(object, SR = NULL, type = "direct", se.fit = 10000, alpha = 0.95, seed = 42,...){
  ...
  if(is.list(preds)){
    ...
    predSR  <-  pmax(poisbinom(probs), 1e-15)
    ...
    predSR.sim <- pmax(poisbinom(probs.sim), 1e-15) # ad-hoc underflow fix
    
    center = predSR[i,]         # point estimate for site i
    dists = hilbert_to_provided_center(predSR.sim, center)
...
gllvm.presence.prob <- function(preds, object, spp = NULL) {
  ...
  for(j in spp){
  if(family[j] %in% c("binomial", "ZIB", "ZNIB"))probs[,j] <- preds[, j]
  if(family[j] == "poisson") {
    probs[,j] <- ppois(matrix(0, nrow = n, ncol = 1), lambda = preds[,j], lower.tail = FALSE)
  } else if(family[j] == "negative.binomial") {
    size <- 1 / rep(object$params$phi[j], n)
    probs[,j] <- pnbinom(rep(0,n), mu = preds[,j], size = size, lower.tail = FALSE)
  } else if(family[j] == "negative.binomial1") {
    size <- preds[,j] * object$params$phi[j]
    probs[,j] <- pnbinom(rep(0, n), mu = preds, size = size, lower.tail = FALSE)
  } else if(family[j] == "betaH") {
    probs[,j] <- 1 - preds[,-seq_len(ncol(object$y)), drop = FALSE][,j]
  } else if(family[j] == "orderedBeta"){
    probs[,j] <- 1 - binomial(link = object$link)$linkinv(binomial(link = object$link)$linkfun(preds[,j]) - object$params$zeta[j,1])
  }else if(family[j] == "ZIP") {
    probs[,j] <- 1 - pzip(rep(0, n), mu = preds[,j], sigma = rep(object$params$phi[j], each = n))
  } else if(family[j] == "ZINB") {
    probs[,j] <- 1 - pzinb(rep(0, n), mu = preds[,j],
                              p = rep(object$params$phi[j], each = n),
                              sigma = rep(object$params$ZINB.phi[j], each = n))
  } else if(family[j] == "tweedie") {
    probs[,j] <- 1 - fishMod::pTweedie(rep(0, n), mu = preds[,j],
                                   phi = rep(object$params$phi[j], each = n),
                                   p = object$Power)
  } else if(family[j] == "ordinal") {
    probs[,j] <- 1 - preds[1,, drop = FALSE]
  }
  }
