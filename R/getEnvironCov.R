...
getEnvironCov.gllvm <- function(object){
...
if(object$col.eff$col.eff=="random"){
  C <- kronecker(diag(ncol(object$y)),t(rep(1,nrow(object$params$Br))))
  ...
  cov.environ.randomB <- sapply(1:(object$num.lv.c+object$num.RR), function(q)object$params$theta[,q,drop=F]%*%t(object$params$theta[,q,drop=F])*object$params$sigmaLvXcoef[q]^2,simplify=F)
  ...
  if(!isFALSE(object$quadratic)){
    ...
    # Sigmaz <- diag(object$params$sigmaLvXcoef^2)
    theta <- object$params$theta[,-c(1:(object$num.lv.c+object$num.RR+object$num.lv))][,1:(object$num.lv.c+object$num.RR)]
    cov.environ.randomB.quad <- sapply(1:(object$num.lv.c+object$num.RR), function(q)2*abs(theta)[,q,drop=F]%*%t(abs(theta[,q,drop=F]))*object$params$sigmaLvXcoef[q]^2,simplify=F)
    trace.environ.randomB.quad <- lapply(cov.environ.randomB.quad, function(x)sum(diag(x)))
  }
  }else if(object$randomB=="P"){
  cov.environ.randomB <- sapply(1:length(object$params$sigmaLvXcoef), function(q)object$params$theta[,1:(object$num.lv.c+object$num.RR),drop=F]%*%t(object$params$theta[,1:(object$num.lv.c+object$num.RR),drop=F])*object$params$sigmaLvXcoef[q]^2,simplify=F)
  trace.environ.randomB <- lapply(cov.environ.randomB, function(x)sum(diag(x)))
  
  if(!isFALSE(object$quadratic)){
    # add tr(D_jSigma_zD_kSigma_z) for z = B^t x
    Sigmaz <- diag(rep(rep(1,ncol(object$lv.X))%*%diag(model2$params$sigmaLvXcoef^2)%*%rep(1,ncol(object$lv.X)),object$num.lv.c+object$num.RR))
    theta <- object$params$theta[,-c(1:(object$num.lv.c+object$num.RR+object$num.lv))][,1:(object$num.lv.c+object$num.RR)]
