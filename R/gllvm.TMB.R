...
gllvm.TMB <- function(y, X = NULL, lv.X = NULL, formula = NULL, lv.formula = NULL, num.lv = 2, num.lv.c = 0, num.RR = 0, family = "poisson",
  ...
  while(n.i <= n.init){
    ...
    if(((method %in% c("VA", "EVA")) && (nlvr>0 || row.eff == "random"))){
      ...
      if(diag.iter>0 && Lambda.struc=="unstructured" && nlvr>1 && !inherits(optr,"try-error")){
        ...
        if(optimizer=="nlm"){
          if(inherits(optr, "try-error") || is.nan(optr$minimum) || is.na(optr$minimum)|| is.infinite(optr$minimum) || optr$minimum < 0){optr=optr1; objr=objr1; Lambda.struc="diagonal"}
  ...
  if(optimizer%in%c("nlminb","optim")){
    ...
    out$convergence <- optrFinal$code %in% c(1,2)
  }
