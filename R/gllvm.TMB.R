...
gllvm.TMB <- function(y, X = NULL, lv.X = NULL, formula = NULL, family = "poisson", 
  ...
  if(col.eff != "random"){
    ...
    if(!is.null(colMat)  && all(dim(colMat)!=1)){
      ...
      if(sp.Ar.struc!="unstructured"){
        ...
        Abranks <- ifelse(sp.Ar.struc.rank>(sum(nsp)*blocksp),sum(nsp)*blocksp,sp.Ar.struc.rank)
      }
    }else{
      colMat <- matrix(0)
      blocks <- list(matrix(0))
  ...
  out <- list( y = y, X = X, logL = Inf, num.lv = num.lv, num.lv.c = num.lv.c, row.eff = row.eff, col.eff = col.eff, colMatBlocksI = blocks, nsp = nsp, family = family, X.design = X, method = method, zeta.struc = zeta.struc, Ntrials = Ntrials)
