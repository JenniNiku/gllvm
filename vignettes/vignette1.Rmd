...
* maximizing an approximation to the log-likelihood that is (almost completely) closed form. We provide two ways to do this -- using Gaussian variational approximations for example overdispersed counts, binary and ordinal responses etc, or using either extended variational approximation or Laplace approximations for these and other distributions when a fully closed form variational approximation cannot be obtained *e.g.* percent cover can be modelled by the beta distribution and beta models adjusted for zeros and ones.
...
| Response    | Distribution | Link | Method | Description |
| ----------- |:------------:|:---- |:------ |:----------- |
| Counts 	| Poisson 		| log 		| VA/EVA/LA 	| $E(y_{ij}) = \mu_{ij}$, $V(\mu_{ij})=\mu_{ij}$ |
| 		| NB 			| log 		| VA/EVA/LA 	| $E(y_{ij}) = \mu_{ij}$, $V(\mu_{ij})=\mu_{ij}+\phi_j\mu_{ij}^2$, |
| 		| NB1 			| log 		| VA/LA 	| $E(y_{ij}) = \mu_{ij}$, $V(\mu_{ij})=\mu_{ij}+\phi_j\mu_{ij}$, |
| 		| 			| 		| 		| where $\phi_j>0$ is a dispersion parameter |
| 		| ZIP 			| log 		| VA/EVA/LA 	| $E(y_{ij}) = (1-p_j)\mu_{ij}$, $P(y_{ij}=0)=p_j$, |
| 		| 			| 		| 		| $V(\mu_{ij}) = \mu_{ij}(1-p_j) (1+\mu_{ij}p_j)$ |
| 		| ZINB 			| log 		| VA/EVA/LA 	| $E(y_{ij}) = (1-p_j)\mu_{ij}$, $P(y_{ij}=0)=p_j$, |
| 		| 			| 		| 		| $V(\mu_{ij}) = \mu_{ij}(1-p_j)(1+\mu_{ij}(p_j+\phi_j))$ |
| 		| binomial 		| probit/logit 	| VA/EVA/LA 	| $E(y_{ij}) = N_j\mu_{ij}$, $V(\mu_{ij}) = N_j\mu_{ij}(1-\mu_{ij})$ |
| 		|  			| cloglog 	| VA/LA 	| |
| zero-inflated | ZIB 			| probit/logit 	| VA/LA 	| $E(y_{ij}) = (1-p_j)N_j\mu_{ij}$, $V(\mu_{ij}) = N_j\mu_{ij}(1-p_j) (1+N_j\mu_{ij}p_j)$ |
| 		|  			| cloglog 	| VA 		| |
| zero/N-inflated | ZNIB 		| probit/logit 	| VA/LA 	| $E(y_{ij}) = p_j^N N_j + (1-p_j^0-p_j^N)N_j\mu_{ij}$, $V(\mu_{ij}) = p_j^N N_j^2 + (1-p_j^0-p^{N_j}) N_j\mu_{ij}(1-\mu_{ij}+N_j\mu_{ij})-E[Y_{ij}]^2$ |
| 		|  			| cloglog 	| VA 		| |
| Overdispersed counts | beta-binomial 	| probit/logit/cloglog | LA 	| $E(y_{ij}) = N_j\mu_{ij}$, $V(\mu_{ij}) = N_j\mu_{ij}(1-\mu_{ij})(1+(N_j-1)\phi_j/(\phi_j+1))$, |
| 		| 			| 		| 		| where $\phi_j > 0$ is a species-specific overdispersion parameter |
| Binary 	| Bernoulli 		| probit/logit 	| VA/EVA/LA 	| $E(y_{ij}) = \mu_{ij}$, $V(\mu_{ij}) = \mu_{ij}(1-\mu_{ij})$ |
| 		| 			| cloglog 	| VA/LA 	| |
| Biomass 	| Tweedie 		| log 		| VA/EVA/LA 	| $E(y_{ij}) = \mu_{ij}$, $V(\mu_{ij}) = \phi_j\mu_{ij}^\nu$, |
| 		| 			| 		| 		| where $1<\nu<2$ is a power parameter and $\phi_j>0$ is a dispersion parameter |
| Ordinal 	| Multinomial 		| probit 	| VA 		| Cumulative probit/logit/cloglog model |
| 		| 			| logit 	| VA/EVA 	| |
| 		| 			| cloglog 	| VA 		| |
| Normal 	| Gaussian 		| identity 	| VA/EVA/LA 	| $E(y_{ij}) = \mu_{ij}$, $V(y_{ij}) = \phi_j^2$ |
| Positive continuous | Gamma 		| log 		| VA/EVA/LA 	| $E(y_{ij}) = \mu_{ij}$, $V(y_{ij}) = \mu_{ij}^2/\phi_j$ |
| 		| 			| 		| 		| where $\phi_j$ is a shape parameter |
| Positive continuous | Exponential 	| log 		| VA/LA 	| $E(y_{ij}) = \mu_{ij}$, $V(y_{ij}) = \mu_{ij}^2$ |
| Percent cover | beta 			| probit/logit 	| EVA/LA 	| $E(y_{ij}) = \mu_{ij}$, $V(\mu_{ij}) = \mu_{ij}(1-\mu_{ij})/(1+\phi_j)$ |
| Percent cover with zeros/ones | ordered beta 	| probit 	| VA-EVA | details in Korhonen et al (2024) |
| 		| ordered beta 		| logit 	| VA-EVA/EVA 		|  |
| 		| beta hurdle 		| probit/logit 	| EVA/VA-EVA 	| |

It is also possible to combine multiple response types in one model, for example counts, binary and ordinal data, and choose suitable family for each response variable by giving a vector of families to argument `family`, for example `family = c("poisson", "binomial", "ordinal")`. There is a restriction for families and links in the same model such that they all need to be implemented with same method, VA, EVA or LA.
...
<img src="figure/F4coef.png" alt="plot of chunk F4 1"  />
<p class="caption"> coefplot</p>
</div>

<div class="figure" style="text-align: center">
<img src="figure/F4.png" alt="plot of chunk F4 2"  />
