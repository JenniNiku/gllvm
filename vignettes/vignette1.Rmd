...
Similarly to the biplot, the species names in the coefficient plot can also be changed by changing the row names in the coefficient matrix. Also, species ordering according to the effects size is optional.
```{r, fig.show='hold', out.width='49%'}
rownames(fit_env$params$Xcoef) <- paste("spp", 1:ncol(fit_env$y), sep = "")
coefplot(fit_env, cex.ylab = 0.7, xlim.list = list(NULL, NULL, c(-4, 4)), 
         order = FALSE, mfrow=c(1,1))
```
...
fit_1th <- gllvm(y, X, TR, family = "negative.binomial", num.lv = 2, 
...
load(file = "fit_1th.RData")
```
```{r, fig.show='hold', out.width='49%', warning = TRUE}
library(lattice)
coefplot(fit_1th, mar = c(4, 11, 1, 1), cex.ylab = 0.8)
fourth <- fit_1th$fourth.corner
a <- 1.5
