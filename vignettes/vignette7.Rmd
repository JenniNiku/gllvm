...
In this vignette we demonstrate how to fit models that incorporate phylogenetic information into the random effects. For this, we make use of an adjusted version of the familiar \texttt{lme4}-formula interface [@lme4].
...
TMB::openmp(parallel::detectCores()-1) # set parallel computation
...
Arguments to control the VA approximation are `Ab.struct` and `Ab.struct.rank`, for details see @vanderVeen2024 (but, note that changing both of these options will likely cause a significant increase in memory use when combined with parallel computation).
