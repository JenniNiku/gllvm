...
```{r}
randomCoefplot(RRGLMb1)
```


When using a random-effects formulation, there are is no summary table or there are confidence intervals available for the canonical coefficients. It is possible to construct a summary table as follows:

```{r eval=T, echo=T}
# Get coefficients
coefs<-data.frame(coef(RRGLMb1,"LvXcoef"))
coefs$Predictor <- row.names(dat)
