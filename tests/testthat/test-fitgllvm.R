...
test_that("fourth corner models works", {
  ...
  ff0<-gllvm(y, X, TR=TR, family = "negative.binomial", seed = 999, sd.errors = FALSE, control = list(max.iter = 1, maxit = 1))
  ff1<-gllvm(y, X, TR=TR, formula = ~pH + pH:t1, family = "negative.binomial", seed = 999, sd.errors = FALSE, control = list(max.iter = 1, maxit = 1))
...
test_that("binomial works", {
  ...
  y01<-(y>0)*1
  fb0<-gllvm(y01, family = binomial(link = "logit"), seed = 999, method = "LA", num.lv = 1, sd.errors = FALSE, control = list(max.iter = 1, maxit = 1))
...
test_that("ZIP works", {
  ...
  fz0<-gllvm(y, family = "ZIP", seed = 999, method = "LA", num.lv = 1, sd.errors = FALSE, control = list(max.iter = 1, maxit = 1))
...
test_that("constrained ordination models work", {
  ...
  fc1<-gllvm(y, X, num.RR = 2, family = "poisson", seed = 999, randomB="LV", sd.errors = FALSE, control = list(max.iter = 1, maxit = 1))
  fc2<-gllvm(y, X, num.RR = 2, family = "poisson", seed = 9, randomB="LV", row.eff="random", sd.errors = FALSE, control = list(max.iter = 1, maxit = 1))
...
test_that("concurrent ordination models work", {
  ...
  X <- scale(spider$x)
  y <- spider$abund
  fc0<-gllvm(y, X, num.lv.c = 2, family = "poisson", seed = 999, sd.errors = FALSE, control = list(max.iter = 1, maxit = 1))
  fc1<-gllvm(y, X, num.lv.c = 2, family = "poisson", seed = 999, randomB="LV", sd.errors = FALSE, control = list(max.iter = 1, maxit = 1))
...
test_that("phylogenetic models work", {
  ...
  expect_error({model<-gllvm(spider$abund,X=X,formula=~diag(ConWate+CovMoss|1),family="poisson",colMat=list(colMat,dist=dist),num.lv=0, sd.errors = FALSE)})
  
  colnames(colMat)<-row.names(colMat)<-colnames(dist)<-row.names(dist)<-colnames(spider$abund)
  expect_error({model<-gllvm(spider$abund,X=X,formula=~diag(ConWate+CovMoss|1),family="poisson",colMat=colMat,colMat.rho.struct="term",num.lv=0, sd.errors=FALSE)})
  
  expect_warning({model<-gllvm(spider$abund,X=X,formula=~diag(ConWate+CovMoss|1),family="poisson",colMat=colMat,Ab.struct="diagonal", num.lv=0, sd.errors = FALSE)})
